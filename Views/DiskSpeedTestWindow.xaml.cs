...
{
    public partial class DiskSpeedTestWindow : Window
    {
        ...
        private const double MAX_EXPECTED = 3500.0; // MB/s (NVMe cap for bar scaling)
        private string? _tempFile;

        public DiskSpeedTestWindow()
        {
            InitializeComponent();
        }
        ...
        private async void StartTest(object sender, RoutedEventArgs e)
        {
            ...
            _tempFile = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "laplap_disktest.bin");
            ...
            try
            {
                do
                {
                    ...
                    Dispatcher.Invoke(() => UpdateUI(writeSpeed, readSpeed));
        ...
        {
            byte[] data = new byte[4 * 1024 * 1024]; // 4MB chunks
        ...
        private void UpdateUI(double writeSpeed, double readSpeed)
        {
            // Update texts
        ...
        private void DrawChart()
        {
            ...
            {
                double x = i * step;
                wp.Add(new System.Windows.Point(x, h - (_writeHistory[i] / maxVal * h * 0.90)));
                rp.Add(new System.Windows.Point(x, h - (_readHistory[i]  / maxVal * h * 0.90)));
            }
