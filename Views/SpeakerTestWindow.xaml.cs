...
namespace LapLapAutoTool.Views
{
    public partial class SpeakerTestWindow : Window
    {
        ...
        // Keep the path of the currently loaded audio (downloaded or opened)
        private string? _currentAudioPath;
        private static readonly HttpClient _http = new() { Timeout = TimeSpan.FromMinutes(5) };
        // Default audio URL used when no link is provided
        ...
        public SpeakerTestWindow()
        {
            ...
            // Set default audio URL so user can just click download
        ...
        private void PlayToneClick(object sender, RoutedEventArgs e)
        {
            if (sender is System.Windows.Controls.Button btn &&
                double.TryParse(btn.Tag?.ToString(), out double freq))
            {
                StopCurrent();
                _waveOut = new WaveOutEvent();
                var gen = new SignalGenerator(44100, 1) // explicitly 1 channel (Mono)
                {
                    Frequency = freq,
                    Gain = 0.5,
        ...
        private void PlayAudioSide(bool left, bool right, string label)
        {
            ...
            try
            {
                ...
                // Use custom provider to mute left or right channel
        ...
        private async void DownloadAndPlay(object sender, RoutedEventArgs e)
        {
            ...
            // If no URL provided, use default audio
        ...
        private void PlayLocalFile(string path)
        {
            ...
            try
            {
                ...
                // store current path for left/right playback
        ...
        private void VolumeChanged(object sender, System.Windows.RoutedPropertyChangedEventArgs<double> e)
        {
    ...
    internal static class SampleProviderExtensions
    {
        ...
        {
            // Use MonoToStereoSampleProvider then pan
            var stereo = new MonoToStereoSampleProvider(mono)
            {
                LeftVolume = leftGain,
                RightVolume = rightGain
            };
            return stereo;
        }
    }
